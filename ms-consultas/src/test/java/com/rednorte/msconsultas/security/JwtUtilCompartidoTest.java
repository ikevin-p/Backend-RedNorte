...
/**
 * Tests unitarios de JwtUtilCompartido.
 *
 * No requiere contexto de Spring: se generan tokens reales con la misma
 * clave secreta y libreria (jjwt) que usa la clase bajo prueba, para
 * verificar extraccion de claims, deteccion de expiracion y manejo de
 * tokens invalidos sin depender de infraestructura externa.
 */
@DisplayName("Tests unitarios — JwtUtilCompartido")
class JwtUtilCompartidoTest {

    // Debe coincidir exactamente con el SECRET privado de la clase bajo prueba.
    ...
    void estaExpirado_tokenVencido_haceQueEsValidoSeaFalse() {
        // Nota: la libreria jjwt lanza ExpiredJwtException al *parsear* un token
        // ya vencido (no solo al leer su fecha de expiracion), por lo que
        // estaExpirado() nunca se invoca de forma aislada sobre un token vencido
        // en el flujo real: esValido() ya captura esa excepcion. Se prueba aqui
        // el comportamiento end-to-end que JwtAuthFilter realmente usa.
